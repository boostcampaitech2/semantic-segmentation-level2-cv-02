    ...
    with tqdm(trn_dl, total=len(trn_dl), unit="batch") as trn_bar:
        for batch, sample in enumerate(trn_bar):
            ...
            if (batch + 1) % (int(len(trn_dl) // 10)) == 0 or (batch + 1) == len(trn_dl):
                logger.info(
                    f"Train Epoch {epoch+1} ==>  Batch [{str(batch+1).zfill(len(str(len(trn_dl))))}/{len(trn_dl)}]  |  Loss: {np.mean(trn_losses):.5f}  |  mIoU: {trn_mIoU:.5f}"
                )
    ...
    with torch.no_grad():
        with tqdm(val_dl, total=len(val_dl), unit="batch") as val_bar:
            for batch, sample in enumerate(val_bar):
                ...
                val_bar.set_postfix(val_loss=np.mean(val_losses), val_mIoU=val_mIoU)
    ...
    with tqdm(pseudo_dl, total=len(pseudo_dl), unit="batch") as pseudo_bar:
        for batch, sample in enumerate(pseudo_bar):
            ...
            if (batch + 1) % (int(len(pseudo_dl) // 10)) == 0 or (batch + 1) == len(pseudo_dl):
                logger.info(
                    f"Train Epoch {epoch+1} ==>  Batch [{str(batch+1).zfill(len(str(len(pseudo_dl))))}/{len(pseudo_dl)}]  |  Loss: {np.mean(trn_losses):.5f}  |  mIoU: {pseudo_mIoU:.5f}"
                )

            pseudo_bar.set_postfix(pseudo_loss=np.mean(trn_losses), pseudo_mIoU=pseudo_mIoU)
